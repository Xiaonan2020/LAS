...
class Convolution_Block(nn.Module):
    ...
    def forward(self, inputs): # inputs:torch.Size([64, 257, 509]) out:torch.Size([64, 64, 255])
        #print(inputs.shape) # b d t
        out = self.conv(inputs)
        #print(out.shape)
        return out # [64,64,265] [16 64 161]
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, input_lengths):
        #print(inputs.shape) # [64,257,529] [64]
        output_lengths = self.get_conv_out_lens(input_lengths).to('cpu') #[64]
        out = self.conv(inputs) 
        #print(out.shape)
        out = out.permute(0,2,1) # []64,265,64] B T D
        #print(out.shape)
        ...
        out, rnn_hidden_state = self.rnn(out) # rnn_hidden_state [4 16 128]

        out, _ = nn.utils.rnn.pad_packed_sequence(out) # 把压缩的还原回来，pad该还还得还
        #print(out.shape) # out: [181 16 128] t b d
        rnn_out = out.transpose(0, 1)
        #print(rnn_out.shape) # b t d
