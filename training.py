...
# 是围绕本机多处理模块的包装器。它注册自定义约简器，使用共享内存在不同进程中提供对相同数据的共享视图。一旦张量/存储被移动到shared_memory(请参阅share_memory_())，就可以将它发送到其他进程，而不需要进行任何复制
...
def train(model,dataloader_train,epoch,optimizer,device,criterion,char_dict,LOG_FILENAME):
    ...
    for i_batch, sample_batched in enumerate(dataloader_train):
        input_features = torch.stack(sample_batched[0]) # B D T [64,257,529]
        input_lengths = torch.from_numpy(np.asarray(sample_batched[1])) # [64]
        targets = torch.stack(sample_batched[2]).long() # [64,100]
        ground_truth_text = sample_batched[3] #[64] [['<s>', 'f', 'a',...], [], ..., []]
        ...
        optimizer.zero_grad()
        decoder_outputs,sequence_symbols = model(input_features,input_lengths,targets, teacher_forcing_ratio=1)
        # decoder_outputs:[16 30 100], sequence_symbols:[16 100]
        if decoder_outputs.size(2) != targets.size(1):
            targets = targets[:,:decoder_outputs.size(2)-targets.size(1)]
