...
class Feature_loader(object):
    ...
    def load_charmap_seq(self):
        ...
        return char_map_list # [2,9,...,1] #80

    def load_char_seq(self):
        chars_list = [str(item)  for item in ' '.join(self.chars.split(' _ ')).split(' ')]
        return chars_list # ['<s>', 'f',... , '</s>']
    
    def load_dataset(self):
        ### Audio processing
        audio_data = self.load_audio()
        features = self.feature_extraction(audio_data)#频域特征
        mag, _ = librosa.magphase(features)  # 计算幅值和相位值 [447,257]
        mag_T = mag.T #[257,447]
        spec_mag = mag_T
        # preprocessing, subtract mean, divided by time-wise var
        mu = np.mean(spec_mag, 0, keepdims=True) #[1,447]
        std = np.std(spec_mag, 0, keepdims=True) #[1,447]
        norm_spec = (spec_mag - mu) / (std + 1e-5) #[257,447]
        
        ### Text processing
        char_map_list = self.load_charmap_seq() #[2,8,...,]
        char_list = self.load_char_seq() # ['<s>', 'f',...,'o', 'r', '</s>']
